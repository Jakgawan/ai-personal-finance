...
export default function BalanceSheetPage() {
  ...
  const openAddLiab = () => {
    setEditLiab(null)
    setLiabName(""); setLiabBalance("")
    setShowLiabModal(true)
  }

  const openEditLiab = (l: Liability) => {
    setEditLiab(l)
    setLiabName(l.name); setLiabBalance(String(l.balance))
    setShowLiabModal(true)
  }
  ...
  const saveLiab = async () => {
    ...
    if (!user) return
    if (editLiab) {
      await supabase.from("liabilities_long").update({ name: liabName, balance: Number(liabBalance), updated_at: new Date().toISOString() }).eq("id", editLiab.id)
    } else {
      await supabase.from("liabilities_long").insert({ user_id: user.id, name: liabName, balance: Number(liabBalance) })
    }
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: "สินทรัพย์ทั้งหมด", value: totalAssets, color: "text-[#1D9E75]" },
          { label: "หนี้สินทั้งหมด", value: totalLiabilities, color: "text-[#D85A30]" },
          { label: "ความมั่งคั่งสุทธิ", value: netWorth, color: netWorth >= 0 ? "text-[#1D9E75]" : "text-[#D85A30]" },
          { label: "อัตราส่วนหนี้/สินทรัพย์", value: debtRatio, color: "text-[#378ADD]", unit: "%" },
        ].map(card => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            ...
            <button onClick={openAddAsset} className="text-xs bg-[#1D9E75] text-white px-3 py-1 rounded-lg hover:bg-[#178a64]">+ เพิ่ม</button>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            ...
            <button onClick={openAddLiab} className="text-xs bg-[#D85A30] text-white px-3 py-1 rounded-lg hover:bg-red-600">+ เพิ่ม</button>
          </div>
            ...
            liabilities.map(l => (
              <div key={l.id} className="flex items-center justify-between px-5 py-3 border-b border-gray-50">
                <div>
                  ...
                  <p className="text-xs text-gray-400">อัปเดต {new Date(l.updated_at).toLocaleDateString("th-TH")}</p>
                </div>
                <div className="flex items-center gap-3">
      ...
      {showLiabModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl">
            ...
            <div className="flex flex-col gap-3">
              <input placeholder="ชื่อหนี้สิน เช่น ผ่อนรถ" value={liabName} onChange={e => setLiabName(e.target.value)}
                className="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1D9E75]" />
